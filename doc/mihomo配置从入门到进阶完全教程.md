...
proxy-groups:
  - name: "🤖 AI 服务"
    ...
    default-selected: '🇺🇸 美国'                # 通过 default-selected 指定默认节点
  ...
  - name: "✈️ 电报消息"
    <<: *pr
    default-selected: '手动选择'
```

> 注：所有代理组的 proxies 顺序将完全相同，不能再通过排序将第一个节点作为默认节点，应通过 `default-selected` 指定

| 优点 | 缺点 |
|------|------|
| 不重复原则，所有功能组的 proxies 选项统一位置 | 暂无 |
| 不容易漏写；配置极短，适合"懒人"场景 |  |
...
proxy-groups:
  ...
  - { name: "香港|自动", <<: [*auto, *hk] }     # ✅ 第二个  *hk 同时被引入
  - { name: "香港|回退", <<: [*fall, *hk] }     # ✅ icon 和 filter 同时被引入
  - { name: "香港|均衡", <<: [*bal,  *hk] }     # ✅ 同上
```

> 注：mihomo 不支持重复 <<:，应使用 `<<: [*锚点1, *锚点2]` 合并多个锚点，详见上例
> 
> 结论：三层锚点思路非常优雅，是非常推荐的写法。

| 优点 | 缺点 |
|------|------|
| 逻辑分层最清晰，修改基础参数只需改 `area`，改某类型参数只需改 `auto`/`fall` 等 | 新手不友好，调试链路长：第三层锚点可简化代理组配置，但也会将其一拆为二，需来回跳转查看完整配置。此层锚点不会被重复使用，可按需选择 |
| 增加新地区理论上只需加一行 |  |
| icon 参数与 filter 打包，视觉管理更方便 |  |
| 代表了社区对锚点用法的最新探索方向 |  |
...
| 流派三 | proxies 候选列表 | 早期教程 | 短 | ⭐⭐⭐ | ⭐⭐⭐ | 推荐搭配其它流派 |
| 流派四 | provider + group（单行极简） | HenryChiao / YYDS | 最短 | ⭐ | ⭐⭐⭐ | ✅ 大型配置 |
| 流派五 | 三层嵌套（含地区 filter+icon） | 社区理想（issue #2274） | 最短 | ⭐ | ⭐⭐⭐⭐ | ✅ 优雅，优选 |
